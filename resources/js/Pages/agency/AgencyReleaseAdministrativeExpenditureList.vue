  ...
  <div class="wrapper">
    ...
    <div v-if="showEditDialog" class="modal fade show d-block" tabindex="-1" role="dialog" style="z-index: 1055;" @click="closeEditDialog">
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable" role="document" style="z-index: 1055; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); margin: 0; max-height: 90vh;" @click.stop>
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Edit Administrative Expenditure Record</h5>
          ...
          <form @submit.prevent="submitEdit">
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="form-group mb-3">
                    <label for="editAgencyVendor">Agency/Vendor <span class="text-danger">*</span></label>
    ...
    <div v-if="showDeleteDialog" class="modal fade show d-block" tabindex="-1" role="dialog" style="z-index: 1055;" @click="closeDeleteDialog">
      ...
      <div class="modal-dialog modal-dialog-centered" role="document" style="z-index: 1055; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); margin: 0;" @click.stop>
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Are you sure you want to delete this Administrative Expenditure record?</p>
...
const fetchEditBalancedFundAmount = async () => {
  ...
  try {
    let url = `/api/balanced-fund-amount-admin-exp?budget_head=${encodeURIComponent(budgetHead)}&program_division_id=${encodeURIComponent(programDivisionId)}`
    if (editForm.value.id) {
      url += `&exclude_type=administrative-expenditure&exclude_id=${editForm.value.id}`
    }
...
const submitEdit = async () => {
  ...
  try {
    const response = await fetch(`/api/agency-release-administrative-expenditure/${editForm.value.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
    ...
    if (response.ok) {
      await fetchAdminExpList()
      showFlashMessage('success', 'Administrative Expenditure record updated successfully.', 'fas fa-check-circle')
      closeEditDialog()
    } else {
      const errorData = await response.json().catch(() => ({}))
