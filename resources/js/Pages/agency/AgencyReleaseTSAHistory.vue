            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                  <div class="card-title">TSA History</div>
                </div>
                <div class="card-body">
                  <!-- Status filter -->
                  <div class="row mb-3">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                      <button class="btn btn-sm btn-outline-secondary" @click="fetchTSAList">
                  ...
                  <div v-if="isLoading" class="text-center py-4">
                    ...
                    <p class="mt-2 text-muted">Loading TSA history...</p>
                  </div>
...
const filteredList = computed(() => {
  if (!statusFilter.value) {
    return tsaList.value
  }
  const statusNum = parseInt(statusFilter.value, 10)
  return tsaList.value.filter((item) => item.status === statusNum)
})
...
const fetchTSAList = async () => {
  ...
  try {
    const res = await fetch('/api/agency-release-tsa-history')
    if (res.ok) {
      const data = await res.json()
      tsaList.value = data
    } else {
      console.error('Failed to fetch TSA data')
      error.value = 'Failed to fetch TSA history from server'
    }
  } catch (err) {
