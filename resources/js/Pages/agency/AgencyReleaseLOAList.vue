            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                  <div class="card-title">LOA List</div>
                </div>
                <div class="card-body">
                  <!-- Flash Message -->
                  ...
                  <div v-if="isLoading" class="text-center py-4">
                    ...
                    <p class="mt-2 text-muted">Loading LOA data...</p>
                  </div>
                  ...
                  <div v-else>
                    ...
                    <div class="table-responsive">
                      <table class="table table-bordered table-head-bg-primary">
                        ...
                        <tbody>
                          <tr v-for="(item, index) in loaList" :key="item.id" :class="{ 'table-secondary': !item.status }">
    ...
    <div v-if="showReviseDialog" class="modal fade show d-block" tabindex="-1" role="dialog" style="z-index: 1055;" @click="closeReviseDialog">
      ...
      <div class="modal-dialog modal-dialog-centered" role="document" style="z-index: 1055; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); margin: 0;" @click.stop>
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Do you really want to proceed?</p>
            <p class="text-muted small mt-2">This will make the older data status inactive and open the LOA form with these data prefilled.</p>
          </div>
          <div class="modal-footer">
...
const fetchLOAList = async () => {
  ...
  try {
    const res = await fetch('/api/agency-release-loa-list');
    if (res.ok) {
      const data = await res.json();
      loaList.value = data;
    } else {
      console.error('Failed to fetch LOA data');
      error.value = 'Failed to fetch LOA data from server';
    }
  } catch (err) {
    console.error('Error fetching LOA data:', err);
    error.value = 'Network error occurred while fetching LOA data';
  } finally {
...
const confirmRevise = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      window.location.href = `/agency-release-loa?${queryParams.toString()}`;
    } else {
      const errorData = await response.json().catch(() => ({}));
