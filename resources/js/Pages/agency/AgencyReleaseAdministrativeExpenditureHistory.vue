        ...
        <div class="page-inner allinsideform">
          <div class="page-header">
            ...
            <ul class="breadcrumbs mb-3">
              ...
              <li class="nav-item">
                <a href="#">Administrative Expenditure History</a>
              </li>
            </ul>
          </div>
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                  <div class="card-title">Administrative Expenditure History</div>
                </div>
                <div class="card-body">
                  <!-- Status filter -->
                  <div class="row mb-3">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                      <button class="btn btn-sm btn-outline-secondary" @click="fetchAdminExpList">
                  ...
                  <div v-if="isLoading" class="text-center py-4">
                    ...
                    <p class="mt-2 text-muted">Loading Administrative Expenditure history...</p>
                  </div>
                        ...
                        <tbody>
                          ...
                          <tr v-if="filteredList.length === 0">
                            <td colspan="10" class="text-center text-muted py-4">
                              ...
                              No Administrative Expenditure history records found
                            </td>
                          </tr>
                        </tbody>
                      </table>
...
const filteredList = computed(() => {
  if (!statusFilter.value) {
    return adminExpList.value
  }
  const statusNum = parseInt(statusFilter.value, 10)
  return adminExpList.value.filter((item) => item.status === statusNum)
})
...
const fetchAdminExpList = async () => {
  ...
  try {
    const res = await fetch('/api/agency-release-administrative-expenditure-history')
    if (res.ok) {
      const data = await res.json()
      adminExpList.value = data
    } else {
      console.error('Failed to fetch Administrative Expenditure data')
      error.value = 'Failed to fetch Administrative Expenditure history from server'
    }
  } catch (err) {
    console.error('Error fetching Administrative Expenditure data:', err)
    error.value = 'Network error occurred while fetching Administrative Expenditure history'
  } finally {
