...
// Get tentative allocation for a state and PD
const getTentativeAllocation = (stateId, pdId) => {
  if (!reportData.value[stateId] || !reportData.value[stateId][pdId]) {
    return '0.00000'
  }
  const amount = reportData.value[stateId][pdId].tentative_amount || 0
  return formatToFiveDecimals(amount)
}

// Get final allocation for a state and PD
const getFinalAllocation = (stateId, pdId) => {
  if (!reportData.value[stateId] || !reportData.value[stateId][pdId]) {
    return '0.00000'
  }
  const amount = reportData.value[stateId][pdId].amount || 0
  return formatToFiveDecimals(amount)
}
...
const getRelease = (stateId, pdId) => {
  ...
  const amount = reportData.value[stateId][pdId].release || 0
  return formatToFiveDecimals(amount)
}
...
const getExpenditure = (stateId, pdId) => {
  ...
  const amount = reportData.value[stateId][pdId].expenditure || 0
  return formatToFiveDecimals(amount)
}
