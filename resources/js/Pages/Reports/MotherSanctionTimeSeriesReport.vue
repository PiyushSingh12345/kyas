        ...
        <div class="page-inner allinsideform">
          ...
          <div class="page-header">
            ...
            <ul class="breadcrumbs mb-3">
              ...
              <li class="nav-item">
                <a href="#">Mother Sanction Report</a>
              </li>
            </ul>
          </div>
          ...
          <div class="report-header mb-4">
            <div class="d-flex justify-content-between align-items-center">
              <h2 class="report-title">Mother Sanction Summary</h2>
              ...
              <div class="filter-group">
                ...
                <div class="filter-input-wrapper">
                  ...
                  >
                    ...
                    <option value="Available Fund">Available Fund</option>
                    <option value="Total Mother Sanction Amount">Total Mother Sanction Amount</option>
                  </select>
                </div>
              </div>
...
function getMetricClass(metric) {
  if (metric === 'Mother Sanction Amount') return 'metric-area'
  if (metric === 'Available Fund') return 'metric-production'
  if (metric === 'Total Mother Sanction Amount') return 'metric-yield'
  return ''
}
...
function getMetricKey(metric) {
  const map = {
    ...
    'Total Mother Sanction Amount': 'total_mother_sanction_amount'
  }
  return map[metric] || ''
}
...
async function applyFilters() {
  try {
    ...
    const response = await fetch(`/api/mother-sanction-time-series?${params}`)
    if (response.ok) {
...
async function loadInitialData() {
  try {
    const [statesRes, yearsRes] = await Promise.all([
      fetch('/api/states'),
      fetch('/api/mother-sanction-time-series')
    ])
