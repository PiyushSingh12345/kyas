      ...
      <div class="container">
        <div class="page-inner allinsideform">
          <div class="page-header">
            <h3 class="fw-bold mb-3">PD Wise Budget Allocation (AAP) - Budget Heads History</h3>
            <ul class="breadcrumbs mb-3">
              ...
              <li class="separator"><i class="icon-arrow-right"></i></li>
              <li class="nav-item">
                <a href="#">PD Wise Budget Allocation History</a>
              </li>
            </ul>
          </div>
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="col-md-2">
                      <div class="form-group">
                        ...
                        <select class="form-select form-select-sm" id="budgetHeadFilter" v-model="filters.bh_id" @change="applyFilters">
                          <option value="">All Budget Heads</option>
                          <option v-for="bh in budgetHeads" :key="bh.id" :value="bh.id">
                            {{ bh.budget }} - {{ bh.description }}
                          </option>
                        </select>
                      </div>
                    </div>
                    ...
                    <table class="table table-striped table-hover">
                      <thead class="table-head-bg-primary">
                        <tr>
                          ...
                          <th @click="sort('bh_id')" class="sortable">
                            Budget Head
                            <i :class="getSortIcon('bh_id')" class="ms-1"></i>
                          </th>
...
export default {
  name: 'PdWiseBudgetAllocationAAPCentralHistory',
  components: {
    Header,
    Sidebar,
    Footer
  ...
  setup() {
    ...
    const fetchHistoryData = async (pageNumber = 1) => {
      ...
      try {
        ...
        const response = await fetch(`/api/pdwise-aap-allocation-history?${params}`)
    ...
    const fetchBudgetHeads = async () => {
      try {
        const response = await fetch('/api/budget-heads-with-history-pdwise')
        const data = await response.json()
        if (data.success) {
    ...
    const exportData = () => {
      ...
      window.open(`/api/pdwise-aap-allocation-history/export?${params}`, '_blank')
    }
